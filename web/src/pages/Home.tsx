...
const Navbar = ({ user, signOut, onLoginClick }: NavbarProps) => {
                ...
                <div className="md:hidden absolute top-full left-0 right-0 bg-[var(--bg-color)] border-b border-[var(--card-border)] p-4 flex flex-col gap-4 shadow-xl">
                    ...
                    {user ? (
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 py-3 border border-[var(--card-border)] text-[var(--text-color)] rounded-xl font-bold cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-all"
                        >
                            {isKo ? '로그인' : 'Login'}
                        </button>
                    )}
        ...
        <div className="min-h-screen bg-[var(--bg-color)] text-[var(--text-color)] selection:bg-indigo-500/30">
            ...
            {isLoginModalOpen && (
                <LoginModal
                    ...
                    isKo={i18n.language.startsWith('ko')}
                />
            )}
